...
if($_SERVER['REQUEST_METHOD'] === 'POST' && !$csrf_valido){
    $mensagem = "Erro: Requisção invalida (token CSRF ausente ou expirado) . ";
    $tipo_mensagem = "Erro";

}

//CREAT categoria
if($_SERVER['REQUEST_METHOD'] == 'POST' && ($_POST['acao'] ?? '') === 'cria_categoria'){
    ...
    if($nome === ''){
        ...
        if($resultado['status'] === 201){
            $mensagem = "categoria \"" . $nome . "\" cadastra com sucesso!";
            $tipo_mensagem = "sucesso";
        }elseif($resultado['status'] === 401 || $resutaldo=['status'] === 403){
            $mensagem = "Sem permissão para cadrasta categoria (crie uma politica de RLS de INSERT para o papelanon).";
            $tipo_mensagem = "erro";
        }
            ...
            $mensagem = "Erro ao cria cadastra categoria (status " . $resultado['status'] . ").";
            $tipo_mensagem = "erro";
        }
    }
}
...
// ordem=nome.asc equivale a ORDER BY nome ASC - Mantém o <selct em ordem alfabética
